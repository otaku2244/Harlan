...
if STATIC_DIR.is_dir():
    # 新前端是单页应用：只有 index.html + style.css + app.js
    # 旧的 AionsHome 前端整体保留在 app/static_legacy/，不挂载（要对比时手动看）
    app.mount("/static", StaticFiles(directory=str(STATIC_DIR)), name="static")

    # 上传的图片
    _uploads = BASE_DIR / "data" / "uploads"
    _uploads.mkdir(parents=True, exist_ok=True)
    app.mount("/uploads", StaticFiles(directory=str(_uploads)), name="uploads")

    # 旧前端单独挂一个前缀，方便对照排查（不占正式路径）
    _legacy = BASE_DIR / "app" / "static_legacy"
    if _legacy.is_dir():
        app.mount("/legacy", StaticFiles(directory=str(_legacy)), name="legacy")
    ...
    async def pwa_manifest() -> FileResponse:
        target = STATIC_DIR / "manifest.json"
    ...
    async def service_worker() -> Response:
        """返回 204 让浏览器跳过 Service Worker 注册。

        理由：开发阶段 SW 会缓存旧页面，改完刷新看到旧的，非常迷惑；
        我们也没有离线需求。以后要 PWA 时把 sw.js 放进来即可自动提供。
        """
    ...
    # 单页应用：所有已知区块路径都返回同一个 index.html，
    # 由前端根据导航切换（不做服务端路由，少一层状态）。
    @app.get("/chat", include_in_schema=False)
    @app.get("/memory", include_in_schema=False)
    @app.get("/diary", include_in_schema=False)
    @app.get("/moments", include_in_schema=False)
